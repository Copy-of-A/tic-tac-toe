...
const useGame = () => {
  ...
  const [gameHistory, setGameHistory] = useState<Array<Array<null | string>>>([Array((+borderSize) ** 2).fill(null)])
  ...
  const computerHandleClick = () => {
    ...
    const randomNum = Math.floor(Math.random() * Math.floor(_freeSquares.length))
  ...
  const handleClick = (i: number, isSynth?: boolean) => {
    ...
    _squares[i] = isX ? X : O
    setSquares(_squares)
    setGameHistory((prevState) => prevState.concat([_squares]))
    setIsX(!isX)
  ...
  const handleBackToPreviousStep = () => {
    ...
    setIsX(!isX)
    setGameHistory((prevState) => prevState.slice(0, -1))
    switch (lastWinner) {
      case 0:
        setRating((rating) => ({
          ...rating,
